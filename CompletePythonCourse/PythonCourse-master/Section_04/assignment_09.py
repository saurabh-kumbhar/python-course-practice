...
for index in range(len(fr)):
    items = fr[index].split(sep='|')
    value_for_item = d.get(items[0])
    if value_for_item is not None:
        items[0] = value_for_item
    else:
        d[items[0]] = str(int(max(d.values()))+1)
        items[0] = d.get(items[0])
